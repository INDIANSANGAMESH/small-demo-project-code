...
public:
    ...
    void enqueue(string name,string address,int age)
    {
        ...
        {
            node*newnode=new node(name,address,age);
            newnode->data=rear->data+1;  //automkaticaly generating token by add ing one
            rear->next=newnode;
            rear->prev=rear;
            rear=newnode;
        }
...
int main()
{
    ...
    // a menu driven program demo for hospital management
do{
    // since ch===a to encheue
    cout<<"entry or.......";
    cin>>ch;
    switch(ch)
    {
case 'a' :
    ...
    //if ch === s then to dequeue
    case 's':
    {
        q.dequeue();
    }break;

    // to count person sby generated numbner from enq(block)
    case 'c':
    {
        q.count();
    }
